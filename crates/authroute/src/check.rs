...
impl CheckRequest {
    ...
    pub fn from_headers(headers: &HeaderMap) -> Self {
        Self {
            proto: header(headers, "x-forwarded-proto").unwrap_or("https").to_string(),
            host: header(headers, "x-forwarded-host")
                .or_else(|| header(headers, "host"))
                .unwrap_or_default()
                .to_string(),
            uri: header(headers, "x-forwarded-uri").unwrap_or("/").to_string(),
            method: header(headers, "x-forwarded-method").unwrap_or("GET").to_string(),
